...
func CollectTxs(cdc codec.JSONCodec, txJSONDecoder sdk.TxDecoder, moniker, genTxsDir string,
	...
	for _, fo := range fos {
		...
		// TODO abstract out srstaking message validation back to srstaking
		msg := msgs[0].(*srstakingtypes.MsgCreateValidator)
		...
		_, valOk := balancesMap[msg.Creator]
		if !valOk {
			_, file, no, ok := runtime.Caller(1)
			if ok {
				fmt.Printf("CollectTxs, called from %s#%d - %s\n", file, no, msg.Creator)
			}
