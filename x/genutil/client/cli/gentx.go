...
import (
	...
	"github.com/cosmos/cosmos-sdk/x/srstaking/client/cli"
)
...
Example:
$ %s gentx my-key-name --keyring-backend=os --chain-id=test-chain-1 \
    --moniker="myvalidator" \
    --details="..." \
    --security-contact="..." \
    --website="..."
`, version.AppName,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			tmValPubKey, err := cryptocodec.ToTmPubKeyInterface(valPubKey)
			if err != nil {
				return errors.Wrap(err, "failed to tendermint validator public key")
			}
			tmValPubKeyBytes, err := tmjson.Marshal(tmValPubKey)
			if err != nil {
				return errors.Wrap(err, "failed to tendermint validator public key marshal json")
			}
			...
			createValCfg, err := cli.PrepareConfigForTxCreateValidator(cmd.Flags(), moniker, nodeID, genDoc.ChainID, sdk.ValAddress(tmValPubKey.Address()), string(tmValPubKeyBytes))
			if err != nil {
			...
			txBldr, msg, err := cli.BuildCreateValidatorMsgForGenutil(clientCtx, createValCfg, txFactory, true)
			if err != nil {
