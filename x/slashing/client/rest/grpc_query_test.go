...
func (s *IntegrationTestSuite) TestGRPCQueries() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			fmt.Sprintf("%s/srspoa/slashing/signing_infos", baseURL),
			map[string]string{
		...
		},
		{
			...
			fmt.Sprintf("%s/srspoa/slashing/signing_infos/%s", baseURL, consAddr),
			map[string]string{
				grpctypes.GRPCBlockHeightHeader: "1",
			},
			false,
		...
		},
		{
			...
			fmt.Sprintf("%s/srspoa/slashing/signing_infos/%s", baseURL, "wrongAddress"),
			map[string]string{},
			true,
			&types.QuerySigningInfoResponse{},
			nil,
		...
		{
			"params",
			fmt.Sprintf("%s/srspoa/slashing/params", baseURL),
			map[string]string{},
			false,
