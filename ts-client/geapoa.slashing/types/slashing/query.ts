...
export const protobufPackage = "geapoa.slashing";
...
export class QueryClientImpl implements Query {
  ...
  Params(request: QueryParamsRequest): Promise<QueryParamsResponse> {
    ...
    const promise = this.rpc.request("geapoa.slashing.Query", "Params", data);
    return promise.then((data) => QueryParamsResponse.decode(new _m0.Reader(data)));
  }

  SigningInfo(request: QuerySigningInfoRequest): Promise<QuerySigningInfoResponse> {
    const data = QuerySigningInfoRequest.encode(request).finish();
    const promise = this.rpc.request("geapoa.slashing.Query", "SigningInfo", data);
  ...
  SigningInfos(request: QuerySigningInfosRequest): Promise<QuerySigningInfosResponse> {
    ...
    const promise = this.rpc.request("geapoa.slashing.Query", "SigningInfos", data);
